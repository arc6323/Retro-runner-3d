...
# Keep one flight pickup. Its visual size is intentionally untouched.
...
# Keep the flight pickup far away initially; exact distance scheduling below takes over once the run starts.
text = text.replace(
...
# Separate the physical value from the HUD display value.
...
# Fix the main bug: movement of pickups was nested inside the magnet-only condition.
...
needle = '\t\tif g.magnet_remaining > 0.0 and pickup_type == "coin":\n'
pos = text.find(needle)
if pos < 0:
    raise SystemExit("magnet pickup block not found")
# Insert common world motion immediately before hit testing, after the magnet block.
hit_needle = '\t\tvar hit_distance: float = Vector2(pickup.position.x - player_local_x, pickup.position.z - player_z).length()'
pos = text.find(hit_needle, pos)
if pos < 0:
    raise SystemExit("pickup hit test not found")
text = text[:pos] + '\t\tif pickup_type != "flight":\n\t\t\tpickup.position.z += current_speed * delta\n' + text[pos:]

# Exact flight scheduling from the displayed distance counter: 4,000 / 8,000 / 12,000 ... metres.
# The flight pickup is positioned from the target distance every frame, so speed changes cannot drift the schedule.
...
# When the flight pickup is collected, schedule the next one exactly 4 km later.
...
# Passing the pickup without collecting also advances it to the next exact 4 km marker.
...
if 'var physical_speed_kmh: float = float(g.get_meta("current_speed_kmh", 0.0))' in action.read_text(encoding="utf-8"):
    replace_once(
        action,
