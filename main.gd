...
func _process(delta: float) -> void:
    ...
    if building != null:
        ...
        # Дом проехал мимо игрока
        if building.position.z > 12.0:

            building.position.z = -80.0


    # Движение квадроцикла по полосам
    if player != null:
