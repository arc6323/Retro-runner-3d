...
func _initialize() -> void:
    ...
    assert(game.vehicle_visual.name == "StreetQuad", "Должна использоваться импортированная модель Street Quad")
    assert(game.wolf_standing.visible, "На обочине должна отображаться стоячая модель Рэда")
    assert(not game.wolf_riding.visible, "Посадочная модель должна быть скрыта до старта")
    assert(game.front_wheel_pivots.size() == 2, "Должны быть найдены два управляемых передних колеса")
    assert(game.wheel_spin_pivots.size() == 4, "Должны быть найдены четыре вращаемых колеса")

    game._begin_start_sequence()
    game._update_mounting(1.0)
    assert(game.game_state == 2, "После посадки должен начаться выезд")
    assert(game.wolf_riding.visible, "После посадки должна использоваться модель Рэда в водительской позе")
